...
rsp_t= [0,1000]; %use at the beginning this time period to find the corrlations, this could change
rsp_t_rating= [0,1000]; %use at the beginning this time period to find the corrlations, this could change
rsp_t_likert= [0,1000]; %use at the beginning this time period to find the corrlations, this could change
rsp_t_bsl= [-400,0];
...
%         pic_A2  = find((ttl_n_cond(:,1) == 7) & ((ttl_n_cond(:,2) == 9)));
        ...
        for jj=[3,6]
            ...
            val_rat_2AFC(:,jj/3)=nan(length(satcue{jj-1}(:,14)),1);
            val_rat_rank(:,jj/3)=nan(length(satcue{jj-1}(:,14)),1);
            val_rat_total(:,jj/3)=nan(length(satcue{jj-1}(:,14)),1);
            ...
            for i=1:20
                ...
                val_rat_2AFC(ind3,jj/3)= val2AFC;
                val_rat_rank(ind3,jj/3)= valRank;
                val_rat_total(ind3,jj/3)= valTotal;
            ...
            val_A_2AFC(:,jj/3) = nan(length(satcue{jj}(:,14)),1);            % value of image on A given in previous whole 2AFC trials (0 to 1900)
            val_B_2AFC(:,jj/3)= nan(length(satcue{jj}(:,14)),1);            % value of image on B given in previous whole 2AFC trials (0 to 1900)
            val_A_ranking(:,jj/3) = nan(length(satcue{jj}(:,14)),1);         % rank of image on A, calculated after all the choice trials (0 to 20)
            val_B_ranking(:,jj/3) = nan(length(satcue{jj}(:,14)),1);         % rank of image on B, calculated after all the choice trials (0 to 20)
            val_A_total(:,jj/3) = nan(length(satcue{jj}(:,14)),1);       % value of image on A given in previous whole 2AFC trials (0 to 1900)
            val_B_total(:,jj/3) = nan(length(satcue{jj}(:,14)),1);       % value of image on B given in previous whole 2AFC trials (0 to 1900)
            ...
            val_C_2AFC(:,jj/3) = nan(length(satcue{jj}(:,14)),1);            % value of image on A given in previous whole 2AFC trials (0 to 1900)
            val_U_2AFC(:,jj/3) = nan(length(satcue{jj}(:,14)),1);            % value of image on B given in previous whole 2AFC trials (0 to 1900)
            val_C_ranking(:,jj/3) = nan(length(satcue{jj}(:,14)),1);         % rank of image on A, calculated after all the choice trials (0 to 20)
            val_U_ranking(:,jj/3) = nan(length(satcue{jj}(:,14)),1);         % rank of image on B, calculated after all the choice trials (0 to 20)
            val_C_total(:,jj/3) = nan(length(satcue{jj}(:,14)),1);       % value of image on A given in previous whole 2AFC trials (0 to 1900)
            val_U_total(:,jj/3) = nan(length(satcue{jj}(:,14)),1);       % value of image on B given in previous whole 2AFC trials (0 to 1900)
            ...
            for i=1:20
                ...
                val_A_ranking(indA, jj/3) = valRank;
                val_B_ranking(indB, jj/3) = valRank;
                val_A_2AFC(indA, jj/3) = val2AFC;
                val_B_2AFC(indB, jj/3) = val2AFC;
                val_A_total(indA, jj/3) = val2AFCtotal;
                val_B_total(indB, jj/3) = val2AFCtotal;
                
                val_C_rat(indC, jj/3) = valRat;
                val_U_rat(indU, jj/3) = valRat;
                val_C_ranking(indC, jj/3) = valRank;
                val_U_ranking(indU, jj/3) = valRank;
                val_C_2AFC(indC, jj/3) = val2AFC;
                val_U_2AFC(indU, jj/3) = val2AFC;
                val_C_total(indC, jj/3) = val2AFCtotal;
                val_U_total(indU, jj/3) = val2AFCtotal;
        ...
        for unit=1:n_ses_units
            ...
            for stim=1:190
                ...
                mfrB1(stim,1) = length(B1{stim}(1,B1{stim} > rsp_t(1,1) & B1{stim} < rsp_t(1,2))) * (1000/abs(rsp_t(1,2) - rsp_t(1,1))); % Hz of this unit at each stimulus at this time window
                mfrB2(stim,1) = length(B2{stim}(1,B2{stim} > rsp_t(1,1) & B2{stim} < rsp_t(1,2))) * (1000/abs(rsp_t(1,2) - rsp_t(1,1))) ; % Hz of this unit at each stimulus at this time window
                mfrA11(stim,1) = length(A11{stim}(1,A11{stim} > rsp_t(1,1) & A11{stim} < rsp_t(1,2))) * (1000/abs(rsp_t(1,2) - rsp_t(1,1))) ; % Hz of this unit at each stimulus at this time window
                mfrA12(stim,1) = length(A12{stim}(1,A12{stim} > rsp_t(1,1) & A12{stim} < rsp_t(1,2))) * (1000/abs(rsp_t(1,2) - rsp_t(1,1))); % Hz of this unit at each stimulus at this time window
                mfrA21(stim,1) = length(A21{stim}(1,A21{stim} > rsp_t(1,1) & A21{stim} < rsp_t(1,2))) * (1000/abs(rsp_t(1,2) - rsp_t(1,1))) ; % Hz of this unit at each stimulus at this time window
                mfrA22(stim,1) = length(A22{stim}(1,A22{stim} > rsp_t(1,1) & A22{stim} < rsp_t(1,2))) * (1000/abs(rsp_t(1,2) - rsp_t(1,1))) ; % Hz of this unit at each stimulus at this time window
                mfrlikAFC1(stim,1) = length(likAFC1{stim}(1,likAFC1{stim} > rsp_t_likert(1,1) & likAFC1{stim} < rsp_t_likert(1,2))) * (1000/abs(rsp_t_likert(1,2) - rsp_t_likert(1,1))) ; % Hz of this unit at each stimulus at this time window
                mfrlikAFC2(stim,1) = length(likAFC2{stim}(1,likAFC2{stim} > rsp_t_likert(1,1) & likAFC2{stim} < rsp_t_likert(1,2))) * (1000/abs(rsp_t_likert(1,2) - rsp_t_likert(1,1))); % Hz of this unit at each stimulus at this time window
            ...
            for stim=1:60
                ...
                mfrlikRat1(stim,1) = length(likRat1{stim}(1,likRat1{stim} > rsp_t_likert(1,1) & likRat1{stim} < rsp_t_likert(1,2))) * (1000/abs(rsp_t_likert(1,2) - rsp_t_likert(1,1))); % Hz of this unit at each stimulus at this time window
                mfrlikRat2(stim,1) = length(likRat2{stim}(1,likRat2{stim} > rsp_t_likert(1,1) & likRat2{stim} < rsp_t_likert(1,2))) * (1000/abs(rsp_t_likert(1,2) - rsp_t_likert(1,1))); % Hz of this unit at each stimulus at this time window
            ...
            val_C = val_C_2AFC;
            val_U = val_U_2AFC;
            val_rat_afc = val_rat_2AFC;
            
            
            % make correlation matrices for each trial (190 for AFC, and 60 for rating)
            
            [CorrRat(unitnum, :), pvalRat(unitnum, :)] = find_corr_rating (mfrRat1, mfrRat2, val_rat, val_rat_afc);
            [CorrLikRat(unitnum, :), pvalLikRat(unitnum, :)] = find_corr_rating (mfrlikRat1, mfrlikRat2, val_rat, val_rat_afc);
            [CorrA1(unitnum, :), pvalA1(unitnum, :)] = find_corr_2AFC  (mfrA11, mfrA12, val_2AFC, val_A, val_B, val_C, val_U);
            [CorrB(unitnum, :), pvalB(unitnum, :)]   = find_corr_2AFC  (mfrB1,  mfrB2, val_2AFC, val_A, val_B, val_C, val_U);
            [CorrA2(unitnum, :), pvalA2(unitnum, :)]  = find_corr_2AFC  (mfrA21, mfrA22, val_2AFC, val_A, val_B, val_C, val_U);
            [CorrLikAFC(unitnum, :), pvalLikAFC(unitnum, :)] = find_corr_2AFC  (mfrlikAFC1, mfrlikAFC2, val_2AFC, val_A, val_B, val_C_ranking, val_U);
            
            [CorrA1_B(unitnum, :), pvalA1_B(unitnum, :)] = find_corr_2AFC  (mfrA11-mfrB1, mfrA12-mfrB2, val_2AFC, val_A, val_B, val_C, val_U);
            [CorrB_A2(unitnum, :), pvalB_A2(unitnum, :)] = find_corr_2AFC  (mfrB1-mfrA21, mfrB2-mfrA22, val_2AFC, val_A, val_B, val_C, val_U);
            [CorrA1_Babs(unitnum, :), pvalA1_Babs(unitnum, :)]  = find_corr_2AFC  (abs(mfrA11-mfrB1), abs(mfrA12-mfrB2), val_2AFC, val_A, val_B, val_C, val_U);
            [CorrB_A2abs(unitnum, :), pvalB_A2abs(unitnum, :) ] = find_corr_2AFC  (abs(mfrB1-mfrA21), abs(mfrB2-mfrA22), val_2AFC, val_A, val_B, val_C, val_U);
            
            
            
           
            
            [CorrRatbsl(unitnum, :), pvalRatbsl(unitnum, :)] = find_corr_rating (mfrRat1bsl, mfrRat2bsl, val_rat, val_rat_afc);
            [CorrLikRatbsl(unitnum, :), pvalLikRatbsl(unitnum, :)] = find_corr_rating (mfrlikRat1bsl, mfrlikRat2bsl, val_rat, val_rat_afc);
            [CorrA1bsl(unitnum, :), pvalA1bsl(unitnum, :)] = find_corr_2AFC  (mfrA11bsl, mfrA12bsl, val_2AFC, val_A, val_B, val_C, val_U);
            [CorrBbsl(unitnum, :), pvalBbsl(unitnum, :)]   = find_corr_2AFC  (mfrB1bsl,  mfrB2bsl, val_2AFC, val_A, val_B, val_C, val_U);
            [CorrA2bsl(unitnum, :), pvalA2bsl(unitnum, :)]  = find_corr_2AFC  (mfrA21bsl, mfrA22bsl, val_2AFC, val_A, val_B, val_C, val_U);
            [CorrLikAFCbsl(unitnum, :), pvalLikAFCbsl(unitnum, :)] = find_corr_2AFC  (mfrlikAFC1bsl, mfrlikAFC2bsl, val_2AFC, val_A, val_B, val_C_ranking, val_U);
            
            [CorrA1_Bbsl(unitnum, :), pvalA1_Bbsl(unitnum, :)] = find_corr_2AFC  (mfrA11bsl-mfrB1bsl, mfrA12bsl-mfrB2bsl, val_2AFC, val_A, val_B, val_C, val_U);
            [CorrB_A2bsl(unitnum, :), pvalB_A2bsl(unitnum, :)] = find_corr_2AFC  (mfrB1bsl-mfrA21bsl, mfrB2bsl-mfrA22bsl, val_2AFC, val_A, val_B, val_C, val_U);
            [CorrA1_Babsbsl(unitnum, :), pvalA1_Babsbsl(unitnum, :)]  = find_corr_2AFC  (abs(mfrA11bsl-mfrB1bsl), abs(mfrA12bsl-mfrB2bsl), val_2AFC, val_A, val_B, val_C, val_U);
            [CorrB_A2absbsl(unitnum, :), pvalB_A2absbsl(unitnum, :) ] = find_corr_2AFC  (abs(mfrB1bsl-mfrA21bsl), abs(mfrB2bsl-mfrA22bsl), val_2AFC, val_A, val_B, val_C, val_U);
            ...
            [PanovaRat{unitnum}, TanovaRat{unitnum}] = find_ancova_rating (mfrRat1, mfrRat2, val_rat, val_rat_afc);
            [PanovaLikRat{unitnum}, TanovaLikRat{unitnum}] = find_ancova_rating (mfrlikRat1, mfrlikRat2, val_rat, val_rat_afc);
            [PanovaA1{unitnum}, TanovaA1{unitnum}] = find_ancova_2AFC  (mfrA11, mfrA12, val_2AFC, val_A, val_B, val_C, val_U);
            [PanovaB{unitnum}, TanovaB{unitnum}]   = find_ancova_2AFC  (mfrB1,  mfrB2, val_2AFC, val_A, val_B, val_C, val_U);
            [PanovaA2{unitnum}, TanovaA2{unitnum}]  = find_ancova_2AFC  (mfrA21, mfrA22, val_2AFC, val_A, val_B, val_C, val_U);
            [PanovaLikAFC{unitnum}, TanovaLikAFC{unitnum}] = find_ancova_2AFC  (mfrlikAFC1, mfrlikAFC2, val_2AFC, val_A, val_B, val_C_ranking, val_U);
            
            [PanovaA1_B{unitnum}, TanovaA1_B{unitnum}] = find_ancova_2AFC  (mfrA11-mfrB1, mfrA12-mfrB2, val_2AFC, val_A, val_B, val_C, val_U);
            [PanovaB_A2{unitnum}, TanovaB_A2{unitnum}] = find_ancova_2AFC  (mfrB1-mfrA21, mfrB2-mfrA22, val_2AFC, val_A, val_B, val_C, val_U);
            [PanovaA1_Babs{unitnum}, TanovaA1_Babs{unitnum}]  = find_ancova_2AFC  (abs(mfrA11-mfrB1), abs(mfrA12-mfrB2), val_2AFC, val_A, val_B, val_C, val_U);
            [PanovaB_A2abs{unitnum}, TanovaB_A2abs{unitnum}] = find_ancova_2AFC  (abs(mfrB1-mfrA21), abs(mfrB2-mfrA22), val_2AFC, val_A, val_B, val_C, val_U);
            
            
            
           
            
            [PanovaRatbsl{unitnum}, TanovaRatbsl{unitnum}] = find_ancova_rating (mfrRat1bsl, mfrRat2bsl, val_rat, val_rat_afc);
            [PanovaLikRatbsl{unitnum}, TanovaLikRatbsl{unitnum}] = find_ancova_rating (mfrlikRat1bsl, mfrlikRat2bsl, val_rat, val_rat_afc);
            [PanovaA1bsl{unitnum}, TanovaA1bsl{unitnum}] = find_ancova_2AFC  (mfrA11bsl, mfrA12bsl, val_2AFC, val_A, val_B, val_C, val_U);
            [PanovaBbsl{unitnum}, TanovaBbsl{unitnum}]   = find_ancova_2AFC  (mfrB1bsl,  mfrB2bsl, val_2AFC, val_A, val_B, val_C, val_U);
            [PanovaA2bsl{unitnum}, TanovaA2bsl{unitnum}]  = find_ancova_2AFC  (mfrA21bsl, mfrA22bsl, val_2AFC, val_A, val_B, val_C, val_U);
            [PanovaLikAFCbsl{unitnum}, TanovaLikAFCbsl{unitnum}] = find_ancova_2AFC  (mfrlikAFC1bsl, mfrlikAFC2bsl, val_2AFC, val_A, val_B, val_C_ranking, val_U);
            
            [PanovaA1_Bbsl{unitnum}, TanovaA1_Bbsl{unitnum}] = find_ancova_2AFC  (mfrA11bsl-mfrB1bsl, mfrA12bsl-mfrB2bsl, val_2AFC, val_A, val_B, val_C, val_U);
            [PanovaB_A2bsl{unitnum}, TanovaB_A2bsl{unitnum}] = find_ancova_2AFC  (mfrB1bsl-mfrA21bsl, mfrB2bsl-mfrA22bsl, val_2AFC, val_A, val_B, val_C, val_U);
            [PanovaA1_Babsbsl{unitnum}, TanovaA1_Babsbsl{unitnum}]  = find_ancova_2AFC  (abs(mfrA11bsl-mfrB1bsl), abs(mfrA12bsl-mfrB2bsl), val_2AFC, val_A, val_B, val_C, val_U);
            [PanovaB_A2absbsl{unitnum}, TanovaB_A2absbsl{unitnum} ] = find_ancova_2AFC  (abs(mfrB1bsl-mfrA21bsl), abs(mfrB2bsl-mfrA22bsl), val_2AFC, val_A, val_B, val_C, val_U);
...
save AFC_correlations_0_1000_0_2000_0_2000 CorrRat CorrLikRat CorrA1 CorrB CorrA2 CorrLikAFC CorrA1_B CorrB_A2 CorrA1_Babs CorrB_A2abs pvalRat pvalLikRat pvalA1 pvalB pvalA2 pvalLikAFC pvalA1_B pvalB_A2 pvalA1_Babs pvalB_A2abs
save AFC_correlations_0_1000_0_2000_0_2000_400bsl CorrRatbsl CorrLikRatbsl CorrA1bsl CorrBbsl CorrA2bsl CorrLikAFCbsl CorrA1_Bbsl CorrB_A2bsl CorrA1_Babsbsl CorrB_A2absbsl pvalRatbsl pvalLikRatbsl pvalA1bsl pvalBbsl pvalA2bsl pvalLikAFCbsl pvalA1_Bbsl pvalB_A2bsl pvalA1_Babsbsl pvalB_A2absbsl
save AFC_ancova_0_1000_0_2000_0_2000 PanovaRat PanovaLikRat PanovaA1 PanovaB PanovaA2 PanovaLikAFC PanovaA1_B PanovaB_A2 PanovaA1_Babs PanovaB_A2abs TanovaRat TanovaLikRat TanovaA1 TanovaB TanovaA2 TanovaLikAFC TanovaA1_B TanovaB_A2 TanovaA1_Babs TanovaB_A2abs
save AFC_ancova_0_1000_0_2000_0_2000_400bsl PanovaRatbsl PanovaLikRatbsl PanovaA1bsl PanovaBbsl PanovaA2bsl PanovaLikAFCbsl PanovaA1_Bbsl PanovaB_A2bsl PanovaA1_Babsbsl PanovaB_A2absbsl TanovaRatbsl TanovaLikRatbsl TanovaA1bsl TanovaBbsl TanovaA2bsl TanovaLikAFCbsl TanovaA1_Bbsl TanovaB_A2bsl TanovaA1_Babsbsl TanovaB_A2absbsl
...
function [CorrRat, pvalRat]= find_corr_rating (mfrRat1, mfrRat2, val_rat_out, val_rat_in)
...
function  [Corr2AFC, pval2AFC]= find_corr_2AFC (mfr1, mfr2, val_2AFCout, val_A, val_B, val_C, val_U)
