...
% %     judge =  find((ttl_n_cond(:,1) == 7) & (ttl_n_cond(:,2) == 1));    %  All judge pics
    ...
    for unit = 2%:n_ses_units %loop over each cluster in this m
...
%         perittl_judge2=rast_mrg_sess{judge2(unit),12};
        ...
        perittl_choice=rast_mrg_sess_T43{unit,12};
...
%         if total_time >= events_time
            ...
            %compute the random baseline only once for all comparissons
            
            bslshuff = randshuff_hcoeff(nstims,rspdur,rawdata,mfr,fast,nshuff); 
            
            shiftwindows=[1,1,1]; shiftdur=0;
            [response_A1{sess, unit}, response_B{sess, unit}, response_A2{sess, unit}]=hcoeff2AFC(raw, mfr, nstims_2AFC, rsp_2AFC, fast,shiftwindows, bslshuff, shiftdur, perittl_A1,  perittl_B, perittl_A2);
            shiftwindows=[3,5]; shiftdur=500;
            [response_choice1{sess, unit},response_choice2{sess, unit},response_choice3{sess, unit},response_choice4{sess, unit},response_choice5{sess, unit},response_choice6{sess, unit}]=hcoeff2AFC(raw, mfr, nstims_2AFC, rsp_2AFC, fast,shiftwindows, bslshuff, shiftdur, perittl_choice);
            shiftwindows=[3]; shiftdur=500;
            [response_judge1{sess, unit},response_judge2{sess, unit},response_judge3{sess, unit}]=hcoeff2AFC(raw, mfr, nstims_rat, rsp_rating, fast, shiftwindows,bslshuff, shiftdur, perittl_judge, perittl_rating); 
            shiftwindows=[6]; shiftdur=500;
            [response_rating1{sess, unit}, response_rating2{sess, unit},response_rating3{sess, unit},response_rating4{sess, unit}, response_rating5{sess, unit}, response_rating6{sess, unit}]=hcoeff2AFC(raw, mfr, nstims_rat, rsp_rating, fast, shiftwindows,bslshuff, shiftdur, perittl_judge, perittl_rating); 
%             
%         else
...
cd(savehere)
savingname1=['Hcoeff_responses_Rating', pat_ID , '_', region];
savingname2=['Hcoeff_responses_2AFC', pat_ID , '_', region];
save (savingname1, 'response_judge1', 'response_judge2', 'response_judge3', 'response_rating1', 'response_rating2', 'response_rating3', 'response_rating4', 'response_rating5', 'response_rating6')
save (savingname, 'response_A1', 'response_B', 'response_A2', 'response_choice1', 'response_choice2', 'response_choice3', 'response_choice4', 'response_choice5', 'response_choice6')
toc
end
